...
# shown above,
# the second right most digit with the second left most digit of the number
# in the sequence.
...
# solution
def thirt(n: int) -> int:
    subtotal = sum(int(num) * ((10 ** index) % 13) for index, num in enumerate(str(n)[::-1]))

    if n == subtotal:
        return subtotal
    return thirt(subtotal)


# tests
import unittest

class DivisibilityByThirteenTests(unittest.TestCase):
    test_cases = [
        (8529, 79),
        (85299258, 31),
        (5634, 57),
    ...
    def test_divisibility(self):
        for tested, expected in self.test_cases:
            self.assertEqual(thirt(tested), expected)
