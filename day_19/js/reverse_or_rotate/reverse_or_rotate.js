...
const revrot = (str, sz) => {
    ...
    for (let i = 0; i < str.length; i += sz) {
        substringArr.push(str.substring(i, i + sz));
    };

    if (substringArr[substringArr.length - 1].length < sz) {
        substringArr.pop()
    };

    for (let i = 0; i < substringArr.length; i++) {
        const elem = substringArr[i]
        let sumOfCubes = 0;
        
        for (let j = 0; j < elem.length; j++) {
            sumOfCubes += Number(elem[j]) ** 2
        }
        if (sumOfCubes % 2 === 0) {
            substringArr[i] = elem.split('').reverse().join('');
        } else {
            substringArr[i] = elem.substring(1) + elem.substring(0, 1);
        }
    }
