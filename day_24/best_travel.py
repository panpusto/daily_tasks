...
# between these towns. ls = [50, 55, 57, 58, 60]. John is tired of driving and he says to Mary that he
...
# if that sum exists, or otherwise nil, null, None, Nothing, depending on the language. In that case with C, C++, D,
# Dart, Fortran, F#, Go, Julia, Kotlin, Nim, OCaml, Pascal, Perl, PowerShell, Reason, Rust, Scala, Shell, Swift return -1.
#
# Examples:
...
def choose_best_sum(t, k, ls):
    routes_combinations = itertools.combinations(ls, k)
    routes_total_dist = [sum(route) for route in routes_combinations if sum(route) <= t]
    return max(routes_total_dist) if routes_total_dist else None


# tests
import unittest

class BestTravelBasicTests(unittest.TestCase):
